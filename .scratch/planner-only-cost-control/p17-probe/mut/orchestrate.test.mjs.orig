...
// B1-B10 pin the unchanged post-stop lifecycle surface.
{
	...
	assert.equal(request.task.state, "changes_requested", "B2/B4: report and request_changes remain valid");
}
{
...
}
{
	...
	assert.equal(orch.reservations.heldCount(task.taskId), held, "B8: budget refusal does not leak a reservation");
}
{
	const { store, task } = budgetTaskFixture("T-20260908-16c-b9", { tokens: 1, costUsd: 0.01 }, usageFixture(1, 0.01));
	const outcome = await new PlannerOrchestrator({ gitRunner, store }).beginDelegation({ toolCallId: "b9", input: { agent: "reviewer", task: `Review ${task.taskId}` } }, BASE);
	assert.equal(outcome.block, undefined, "B9: reviewer remains exempt after stop");
}
{
