...
用户指出：测试时选的主模型偏便宜，费用要按**主模型 5× 价格**再看一遍。先把口径写死，
因为这一轮所有座位跑的都是同一个模型（`kimi-coding/kimi-for-coding`，只有 thinking 档位
`high`/`medium` 之分），"主模型 ×5" 有两种读法，两个都给出：

| 口径 | root | children(18) | 合计 | root 占比 | 未入账金额 | 未入账占比 |
|---|---|---|---|---|---|---|
| 原始实测 | $0.09911 | $0.68283 | **$0.78194** | 12.7% | $0.27450 | **35.1%** |
| **A：只有 root 按 5×**（主模型＝规划者，子代理视作便宜模型） | $0.49555 | $0.68283 | **$1.17838** | 42.1% | $0.27450 | 23.3% |
| **B：全部座位同一主模型，整体 5×** | $0.49555 | $3.41417 | **$3.90972** | 12.7% | $1.37252 | 35.1% |

对照 r4（同样两个口径）：实测 $0.29582 → A $0.51697 → B $1.47908。
参考值 ~$0.19 在 B 口径下约 ~$0.95。

**结论不变，代价变大。** 缩放是线性的，不动任何一条验收的判定：

- C14 仍然 FAIL。B 口径下漏记比例逐字不变（35.1%），只是绝对金额从 $0.27 变成 **$1.37**；
  A 口径下比例降到 23.3%，但缺口本身（oracle 从不入账 + 末次落账后 7 个子代理丢失）
  一分钱没少 —— 缺口是 children 侧的，抬高 root 单价只是把分母做大，不是修复。
  **不要拿 A 口径的 23.3% 说"账本准了"。**
- 这一轮的超支本来就不是"模型贵"，是根因一的死锁重试循环（50.8 min vs 参考 ~8 min）。
  按 B 口径，那个循环一次烧掉 $3.91，是参考值的 **4.1 倍**；A 口径 $1.18，仍是 1.24 倍
  —— 而且 A 口径下 root 占比从 12.7% 跳到 42.1%，意味着主模型越贵，
  "Root 反复重派、每次都被同一道校验挡回"这件事越贵，工单 27 的优先级越高。

口径来源：children 单价取 `<SA>/*_meta.json` 的 `usage.cost`（18 个全有），
root 取 `usage.jsonl` 末条的 `root.costUsd`；两者与上文时间线表同源，未做任何估算。
