...
只读票面 prompt：`.scratch/planner-only-cost-control/p20-scale/prompt-e2-isolated-baseline.md`
...
- 本轮 `ISO-E2` 用 `slot cpu --`。起重前 `slot audit` + `slot status` 写入
...
跑 **一组** 付费对照：`ISO-E2`（luna 独立执行 F6 E2：status 读取 writeErrorFor）。跑完就停。

## 2. 围栏

[可以改] 无主仓产品文件。
[可以新建] `.scratch/planner-only-cost-control/p20-scale/<round_id>-*.log` 与 `<round_id>-execution-report.md`。
worktree `/home/tcuni-claw/pi/pi-planner-only-p19-experiment` 已存在。planner 已 restore 到基线；内层 `pi` 只应改 worktree 的 `orchestrate.ts` / `orchestrate.test.mjs`。
[不许动] 主仓产品文件、`p19-experiment/` 既有 `runs/`、`p18-contract-run/`、`p20-scale/freeze.md`、`p20-scale/run.sh`、`prompt-e2-isolated-baseline.md`、既有 `session-ISO-E3` / `session-SPLIT-E3` / `session-ABORT-SPLIT-E3`。
不 commit、不 push、不勾 checkbox。
**禁止** `SPLIT-*`、`ISO-E3`、`ISO-E1`、p19 的 38/39、任何第二组。
...
期望 HEAD 为 `45d9493e25f47c58911edc01757c6133caeaa39d`；两文件 **无** tracked diff。若 E3 样本仍在，只允许：
...
```
slot cpu -- bash .scratch/planner-only-cost-control/p20-scale/run.sh ISO-E2
```
...
- 有 `p20-scale/runs/session-ISO-E2`（除非起 pi 之前就撞帽）
- 没有 `session-SPLIT-E2` / `session-ISO-E1`
- 两行 spend.py 原文进报告
...
贴 round_id、起止 HEAD（主仓与 worktree 分开写）、ISO-E2 exit、两行 spend.py、没做到的事、假设。
回报只给路径 + 3–6 行。
