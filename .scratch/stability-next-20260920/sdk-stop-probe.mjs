...
delete process.env.PI_PLANNER_ONLY_ROLE_MODELS;
...
let queuedProviderEntered = false;
...
try {
  ...
  faux.setResponses([bad(1), bad(2), bad(3), () => { queuedProviderEntered = true; return delegate("queued"); }, fauxAssistantMessage("done")]);
  ...
  assert.equal(extraModelCallsAfterClose, 0, "reasserting abort on the queued host run prevents its provider call");
  assert.equal(queuedProviderEntered, false, "queued response never reaches the provider");
  ...
  const result = { status: "PASS", host, hostVersion: JSON.parse(readFileSync(join(host, "package.json"), "utf8")).version,
    modelCallsAtClosure, modelCallsAtFirstSettlement, extraModelCallsAfterClose, queuedProviderEntered, modelCallsTotal: faux.state.callCount, stopped, next: resumed, unknown, repaired, trace,
  ...
  console.log(JSON.stringify({ status: result.status, hostVersion: result.hostVersion, extraModelCallsAfterClose, queuedProviderEntered, modelCallsTotal: result.modelCallsTotal, launches: launches.length, run }));
} finally {
