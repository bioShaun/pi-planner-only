...
const sessions = "/home/tcuni-claw/.codex/sessions/2026/09/20";
const rolloutFor = (id) => fs.readdirSync(sessions).filter((f) => f.endsWith(`${id}.jsonl`)).map((f) => path.join(sessions, f))[0];
...
const residue = execSync("for p in $(pgrep -x codex); do s=$(ps -o lstart= -p $p); [ \"$(date -d \"$s\" +%s)\" -ge \"$(date -d '" + launchedAt + "' +%s)\" ] && echo \"$p $s\"; done; true", { shell: "/bin/bash" }).toString().trim().split("\n").filter(Boolean);
...
const out = {
  ...
  externalTimeoutSeconds: Number(process.env.REVIEW_READONLY_TIMEOUT || 420),
