...
- 独立审核收尾（2026-09-16）：初次实现把普通 `completed` 直接记为 `normal-completion`，与 §3 的 iff 谓词不一致；现已改为所有 writer terminal 均经静滞确认，未确认的 completed 报告不入账并保持隔离。迟到 terminal 保留 monitor 已判定的 `worker_runaway`；wall envelope 严格覆盖 launcher 等待；RecoveryDecision 忽略理由措辞、按规范化 evidenceRefs 等结构判重；`manual` 决策可显式解除已人工处置的 hold；需隔离账本不受普通恢复条数上限影响。生产路径回归与完整 typecheck/test 通过；host-01/02/03 文件保留当时版本的历史观测，不改写。
