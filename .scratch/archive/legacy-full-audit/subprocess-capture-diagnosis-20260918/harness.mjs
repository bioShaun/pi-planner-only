...
const baselineCase = syncCases[0];
const baselineExpectedStdout = 'capture-probe\n';
const baselineFailures = [];
if (baselineCase.error !== null) baselineFailures.push('result.error is present');
if (baselineCase.status !== 0) baselineFailures.push(`status is ${baselineCase.status}`);
if (baselineCase.signal !== null) baselineFailures.push(`signal is ${baselineCase.signal}`);
if (baselineCase.stdout !== baselineExpectedStdout) {
  baselineFailures.push(`stdout is ${JSON.stringify(baselineCase.stdout)}`);
}
const baselineEvaluation = {
  case: baselineCase.name,
  expected: {
    error: null,
    status: 0,
    signal: null,
    stdout: baselineExpectedStdout,
  },
  pass: baselineFailures.length === 0,
...
process.exitCode = baselineEvaluation.pass ? 0 : 1;
