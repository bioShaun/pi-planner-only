...
Independent reviewer’s final message:
...
Parent runtime permission evidence: the inline Python probe completed with exit `0`. Opening `index.ts` using only `os.O_WRONLY` failed with errno `30`, “Read-only file system.” Its covering mount record was:

```text
1644 36 259:2 / / ro,nosuid,nodev,relatime master:1 - ext4 /dev/nvme0n1p2 rw
```

Before and after SHA-256 were identical:
