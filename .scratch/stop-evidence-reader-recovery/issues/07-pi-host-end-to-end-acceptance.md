...
- [ ] 记录实际 Pi、pi-subagents、插件版本/加载指纹、cwd 与受控输入；CLI 验证保留完整实际调用形式，包括要求的 pi --mode json。（缺口：版本号已记录，但完整实际 CLI 调用、退出码与版本/插件加载指纹的命令产物尚未取证）
- [x] 隔离真实用户工作区，不操作 4090 现场数据作为测试夹具。
...
- [x] 运行完成后重载/重启，确认 reader 没有新增 writer 占用。（宿主证据：fresh pi 进程恢复 T-20260917-008 账本记录，未合成 hold）
- [ ] 另用受控无匹配终态的 reader 场景证明停止未知也不合成 Writer hold。（缺口：尚无真实宿主场景，仅自动测试覆盖）
- [x] 用受控 writer 启动前非 Git 环境验证不派发、无新 hold；用启动后出现的采样失败或工作区变化验证 hold 保持、第二 writer 被拒，重启后仍隔离。Validator 不因 readOnly 标志绕过该要求。
- [x] 验证匹配终态、有效静止证据齐备后的正常释放，以及取消、迟到终态不误接纳报告、不重复记账；未验收报告仍可供诊断识别。（取消+迟到终态有第二轮宿主证据 T-20260917-007）
- [ ] 重复终态不误接纳报告、不重复记账。（缺口：仅 delegate.test.mjs 自动测试覆盖——重复终态为 transport 层去重，对宿主不可见，不能描述为真实宿主重复投递证据）
- [x] 委派失败后的 Idle 状态通过 planner_tasks 直接查询对应失败 Execution、报告收到/接纳状态、采样阶段、真实 hold 和日志位置状态。（第二轮宿主验收对 T-20260917-009 执行过查询）
- [ ] 验证查询自身没有另起子任务，也没有改变账本或并发状态。（缺口：尚无查询前后账本及并发零副作用的直接对比）
- [x] 对 observation/worktree 的默认值、历史兼容、角色限制、重绑定拒绝和 Writer 的 Reviewer/Verdict/提交门禁，引用当前实现的自动回归证据；若集成后行为变化，重跑受影响场景。
- [x] 完成当前实现的类型检查和完整发布检查（npm run test:release）。
- [ ] 保存原始命令、退出码、stdout/stderr、失败尝试、修正后的通过证据及状态漂移核对；不只记录最终 exit 0。（缺口：目前仅记录检查通过的结论，完整实际命令产物尚未取证）
...
- [ ] 交付中性证据供 Root 与独立 Reviewer 接受；原生 Codex 有强制 strict gate 时使用项目规定的独立只读入口并证明实际权限，Worker 自测不冒称独立验证。全部条件通过后才报告实现验收完成。（尚未满足：上述缺口补齐前不得报告验收完成）
...
证据文件： `evidence/pi-host-t07-*.{stdout.json,stderr.txt}` + `pi-host-t07-ledger-*.json` + `pi-host-t07-summary.json`. Regression evidence: `delegate.test.mjs`, `orchestrate.test.mjs`, `index.test.mjs`, `task.test.mjs`, `evidence.test.mjs`; `npm run test:release` clean.
...
证据文件： `evidence/pi-host-t07b-*.stdout.json` + `pi-host-t07b-ledger-T-20260917-009.json` + `pi-host-t07b-summary.json`.

## Comments

- Host evidence gaps remain; acceptance is pending the missing independent host evidence.
...
- 2026-09-17 复审修订（按 d1cc34c 复审 REQUEST_CHANGES 如实更正）：第二轮宿主验收补上了启动后证据故障（hash-failed）下 `stop_unconfirmed` + `writerHold` 保留、第二 writer `WORKSPACE_CONFLICT` 拒绝、重启后隔离保持，以及取消与迟到终态（T-20260917-007）的宿主证据。以下缺口仍未补齐：① 无匹配终态 restricted-reader 的真实宿主场景（仅自动测试覆盖）；② planner_tasks 查询前后账本及并发零副作用的直接对比；③ 完整实际 CLI 调用、退出码、版本与插件加载指纹的命令产物；④ 重复终态仅 delegate.test.mjs 自动测试覆盖（transport 去重对宿主不可见，不能描述为真实宿主重复投递证据）。上述缺口补齐前，本票不报告验收完成。
