...
- [ ] 记录实际加载的插件、host、launcher 版本与相关契约，检查 child 首次执行前实际可见的完整上下文；区分“本插件未注入身份”和“launcher 所有渠道均未提供身份”，不以子会话自述代替核对。
- [ ] 明确 Task id、executionId、requestId、ownerRunId、child runId 的意义；本次 runId 由 launcher 权威分配，并通过受支持、确定性的渠道在 child 首次执行前提供。
- [ ] 子会话看到的 runId 与本次 terminal response 一致；纠正轮取得自己的新 runId，不复用前一轮，也不要求 Root 预先猜出该值。
- [ ] 从已注册工具入口驱动 Explorer 与 Worker 的报告接纳；Explorer 覆盖两个 Task、其中一个有纠正轮，三次执行分别准确绑定身份，其他条件满足时正常进入 Verdict。
- [ ] 测试 child 必须从真实规定的下发渠道获得身份，禁止测试代码绕开该渠道预先把相同 runId 塞给 WorkerReport 与 terminal response 后宣称下发通过。
...
- [ ] 在支持的真实 Pi host/launcher 上演示首轮及纠正轮身份下发、报告接纳和 Verdict，保存脱敏关联记录；本票不依赖其他缺陷先修复，可使用容量空闲、参数完整的隔离场景。
...
当前证据边界：已完成安装目录中的 `pi-subagents@0.68.0` 源码检查，尚未取得同一真实宿主会话的加载指纹、provider/model、有效 schema 与 child 可见上下文记录。
...
1. **实际运行版本与上下文核对**：
   ...
   - 当前兼容处理：`pi-planner-only` 只接受 launcher/probe 的生产能力证明；环境变量不能覆盖 false 或缺失的探针。缺少 `childRunIdentity` 时在 Task 准入前抛出 `LAUNCHER_CAPABILITY_UNSUPPORTED`。
   - 上游依赖要求：需要 `pi-subagents` 在 child 首次执行前传入与 terminal response 相同的 runId，并提供有文档的能力广告。尚无版本证据支持指定最低版本。

2. **确定性下发渠道与接线（superseded：仅为测试 fixture，不是已安装 launcher 的生产实现）**：
