				...
				const lines = [
					...
					...d.executions.flatMap((execution) => [
						...
						`  timing: request=${execution.requestId ?? "unknown"} launched=${execution.launchedAt ?? "unknown"} started=${execution.startedAt ?? "unknown"} ended=${execution.endedAt ?? "unknown"} durationMs=${execution.durationMs ?? "unknown"}${execution.durationBasis ? ` (${execution.durationBasis})` : ""}${execution.requestClosed ? ` | requestClosed=${execution.requestClosed} at ${execution.requestClosedAt ?? "unknown"}` : ""}`,
							...
							...(execution.traceSummary ? [`  trace: firstNonReadOnlyToolOrdinal=${execution.traceSummary.firstNonReadOnlyToolOrdinal ?? "none"} maxTokenDelta=${execution.traceSummary.maxTokenDelta ?? 0} readOnlyToolFraction=${execution.traceSummary.readOnlyToolFraction} coverage=${execution.traceSummary.classifiedToolCalls}/${execution.traceSummary.totalToolCalls} observed=${execution.traceSummary.observedToolCalls} coalesced=${execution.traceSummary.coalescedToolCalls}`] : []),
						...(execution.updateTrace ?? []).map((update) => `  trace tool #${update.ordinal} count=${update.toolCount ?? "unknown"}: ${update.currentTool ?? "none active"}${update.currentToolArgs ? ` args=${JSON.stringify(update.currentToolArgs)}` : ""}${update.recentTools?.length ? ` recent=${JSON.stringify(update.recentTools)}` : ""}`),
