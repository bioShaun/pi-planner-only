...
- om09 run4 follow-ups (`.scratch/om09-run4/spec.md`):
  ...
  - Automatic handoff: a `handoff` tool (allowed when the user asked via `/planner-only handoff [goal]`, or when Root's context is above the warning threshold) stores a brief; after the turn the plugin opens a new session (`parentSession` links back) and submits the brief with git facts under a header telling the new Root to do the next step itself. `PI_PLANNER_ONLY_HANDOFF=confirm` puts it in the editor instead. A cancelled or failed handoff keeps the brief for `/planner-only handoff` retry or `handoff drop`. Refused while any delegation runs. Verified live in tmux, including the chain case found in the first live test.
  - `git_audit` treats an empty `path` as no filter.
